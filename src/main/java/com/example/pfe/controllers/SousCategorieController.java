...
public class SousCategorieController {
    ...
    public ResponseEntity<?> createSousCategorie(
            @RequestHeader("Authorization") String token,
            @RequestParam("titreSousCategorie") String titreSousCategorie,
        ...
        try {
            ...
            if (categorieOpt.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Catégorie introuvable");
            }
            ...
            SousCategorie sousCategorie = new SousCategorie(titreSousCategorie, description, imageName, categorieOpt.get());
            SousCategorie createdSousCategorie = sousCategorieService.createSousCategorie(sousCategorie);
            return ResponseEntity.status(HttpStatus.CREATED).body(createdSousCategorie);

        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Erreur lors de la création de la sous-catégorie : " + e.getMessage());
        }
    }
}